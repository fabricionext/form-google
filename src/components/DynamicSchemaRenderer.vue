  ...
  <div class="dynamic-schema-renderer">
    ...
    <div v-else-if="schema" class="space-y-6">
      ...
      <div v-if="schema.title || schema.description" class="border-b pb-4">
        <h2 v-if="schema.title" class="text-2xl font-bold text-gray-900">
          {{ schema.title }}
        </h2>
        <p v-if="schema.description" class="mt-2 text-gray-600">
          {{ schema.description }}
        </p>
      </div>
      ...
      <!-- Dynamic Sections -->
      <div
        v-for="section in organizedSections"
        :key="section.name"
        class="space-y-4"
      >
      ...
      <!-- Conditional Fields -->
      <div
        v-if="conditionalFields.length > 0"
...
const addAuthor = () => {
  ...
  if (currentAuthors < maxAuthors.value) {
    ...
    const authorTemplate = multiAuthorFields.value.filter(f =>
      f.name.startsWith('autor_1_')
    );

    authorTemplate.forEach(templateField => {
      const newFieldName = templateField.name.replace(
        'autor_1_',
        `autor_${newAuthorNum}_`
      );
...
const categorizarCampo = chave => {
  ...
  if (
    chaveLower.includes('orgao_transito') ||
  ...
  if (
    chaveLower.includes('endereco') ||
    chaveLower.includes('logradouro') ||
    chaveLower.includes('numero') ||
    chaveLower.includes('bairro') ||
    chaveLower.includes('cidade') ||
    chaveLower.includes('cep')
  ) {
    return 'endereco';
  }

  if (
    chaveLower.includes('processo') ||
    chaveLower.includes('infracao') ||
    chaveLower.includes('multa') ||
    chaveLower.includes('artigo')
  ) {
    return 'processo';
  }
  ...
  if (
    ...
    chaveLower.includes('cpf') ||
    chaveLower.includes('rg') ||
    chaveLower.includes('profissao')
  ) {
    return 'cliente';
  }
