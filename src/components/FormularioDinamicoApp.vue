<template>
  <div class="formulario-dinamico-app">
    ...
    <div v-if="dataChanged" class="alert alert-info mb-3">
      ...
      <button @click="salvarRascunho" class="btn btn-sm btn-outline-primary ms-2">
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        ...
        <AutoridadeSearch 
          @autoridade-selecionada="handleAutoridadeDrop"
        />
      </div>
...
// Computed do store
const { formData, dataChanged, submitting, camposOrganizados } = formularioStore
...
const handleClienteDropAutor = ({ clienteData, autorIndex }) => {
  ...
  mostrarToast(`Autor ${autorIndex} preenchido com ${clienteData.nome_completo}!`, 'success')
}
...
const handleSubmit = async (event) => {
  ...
  try {
    ...
    Object.keys(formularioStore.formData).forEach(key => {
      if (formularioStore.formData[key] !== undefined && formularioStore.formData[key] !== null) {
