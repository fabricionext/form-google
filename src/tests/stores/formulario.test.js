...
describe('Formulario Store', () => {
  ...
  describe('Document Generation', () => {
    ...
    it('fails when form is invalid', async () => {
      ...
      await expect(store.generateDocument()).rejects.toThrow('Formulário contém erros de validação')
    ...
    it('handles generation errors', async () => {
      ...
      api.formsAPI.validate.mockResolvedValue(mockValidationResponse)
      api.documentsAPI.generate.mockRejectedValue(new Error('Generation failed'))
      
      await expect(store.generateDocument()).rejects.toThrow('Generation failed')
    ...
    it('requires template to generate', async () => {
      ...
      await expect(store.generateDocument()).rejects.toThrow('Template não selecionado')
    })
  })
  ...
  describe('Client Search', () => {
    ...
    it('gets client by CPF', async () => {
      const mockClient = { id: 1, nome_completo: 'João Silva', cpf: '123.456.789-01' }
