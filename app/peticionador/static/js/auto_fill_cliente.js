...
class AutoFillCliente {
  ...
  setupCPFSearch() {
    ...
    const cpfFields = this.findFieldsByPattern([
      'cpf',
      'proprietario_cpf',
      'autor_1_cpf',
    ]);
  ...
  setupNameSearch() {
    ...
    const nomeFields = this.findFieldsByPattern([
      'primeiro_nome',
      'nome',
      'proprietario_nome',
    ]);
    const sobrenomeFields = this.findFieldsByPattern([
      'sobrenome',
      'proprietario_sobrenome',
    ]);
  ...
  async searchByCPF(cpf, triggerField) {
    ...
    try {
      const response = await fetch(
        `/peticionador/api/clientes/busca_cpf?cpf=${cleanCPF}`
      );
      ...
      if (result.success && result.cliente) {
        ...
        this.showToast(
          `Cliente ${result.cliente.nome_completo} carregado!`,
          'success'
        );
      } else {
  ...
  async searchByName(triggerField) {
    ...
    try {
      ...
      const response = await fetch(
        `/peticionador/api/clientes/busca_nome?${params}`
      );
      const result = await response.json();
      ...
      if (result.success && result.clientes && result.clientes.length > 0) {
        if (result.clientes.length === 1) {
          ...
          this.showToast(
            `Cliente ${result.clientes[0].nome_completo} carregado!`,
            'success'
          );
        } else {
  ...
  fillClientData(clienteData, triggerField) {
    ...
    const fieldMapping = {
      ...
      [`${prefix}telefone`]:
        clienteData.telefone_celular || clienteData.telefone_outro,
  ...
  showClientSelector(clientes, triggerField) {
    ...
    modal.querySelectorAll('.list-group-item').forEach((item, index) => {
      item.addEventListener('click', () => {
        ...
        modal.remove();
        this.showToast(
          `Cliente ${clientes[index].nome_completo} selecionado!`,
          'success'
        );
      });
    });
