...
# Script final para iniciar o stack de monitoramento
# Resolve problemas de permissão e montagem

echo "🚀 Iniciando stack de monitoramento (versão final)..."
...
# Remover volumes antigos e criar novos
...
# Iniciar Loki com configuração inline (evita problemas de montagem)
echo "📦 Iniciando Loki..."
docker run -d \
  --name loki \
  --network monitoring \
...
sleep 3
docker cp /tmp/monitoring-config/loki/loki-config.yaml loki:/etc/loki/local-config.yaml
